...
func (h *EventHandler) Run(ctx context.Context) error {
	...
	connector := func() (signalr.Connection, error) {
		conn, err := signalr.NewHTTPConnection(
			...
			signalr.WithHTTPHeaders(func() http.Header {
				header := h.client.getAuthorizationHeader()
				return header
			}),
	...
	client, err := signalr.NewClient(
		ctx, signalr.WithConnector(connector), signalr.WithBackoff(func() backoff.BackOff {
			bo := backoff.NewExponentialBackOff()
			bo.MaxElapsedTime = 3 * 24 * time.Hour
			return bo
		}), signalr.TransferFormat(signalr.TransferFormatText), signalr.WithReceiver(h), signalr.EnableDetailedErrors(true),
	)
